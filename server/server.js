...
function getBrowser() {
  if (!browserPromise) {
    browserPromise = chromium
      ...
      .catch((err) => {
        // jeśli start się nie uda, zresetuj, żeby kolejne zapytanie spróbowało od nowa
        browserPromise = null;
        throw err;
      });
  }
...
app.post("/api/search", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    // jeśli przeglądarka padła (np. brak pamięci), zresetuj ją, żeby kolejne zapytanie ruszyło od nowa
    const msg = String(err.message || err);
    if (/closed|crashed|disconnected|Target page/i.test(msg)) {
      browserPromise = null;
    }
    return res.status(500).json({ error: "Błąd scrapowania: " + msg });
  } finally {
    releaseSlot();
  }
});
