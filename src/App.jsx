...
export default function App() {
  const [backendUrl, setBackendUrl] = useState(""); // adres Twojego wdrożonego backendu scrapera, tylko w pamięci
  ...
  async function lookupOne(queryText, attempt = 0) {
    const response = await fetch(`${backendUrl.replace(/\/$/, "")}/api/search`, {
      method: "POST",
    ...
    if (response.status === 429) {
      ...
      if (attempt >= maxAttempts) {
        throw new Error("Serwer scrapera jest przeciążony (429) — spróbuj ponownie za chwilę");
      }
    ...
    if (response.status >= 500) {
      const maxAttempts = 3;
      if (attempt >= maxAttempts) {
        throw new Error("Błąd serwera scrapera: " + response.status);
      }
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error || "Błąd backendu: " + response.status);
    }
    ...
    return (
      <div style={{ background: PAPER, minHeight: "100%", fontFamily: "'IBM Plex Sans', 'Segoe UI', sans-serif", color: INK, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ background: "#fff", border: `1px solid #ddd`, borderRadius: 6, padding: 32, maxWidth: 420, width: "100%" }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 800, margin: "0 0 10px" }}>Podaj adres backendu</h2>
          <p style={{ fontSize: 13, color: "#555", marginBottom: 16, lineHeight: 1.5 }}>
            To darmowa wersja bez AI — działa przez własny scraper (patrz README, folder <code>server/</code>).
            Wdróż go (np. na Render.com) i wklej tutaj jego publiczny adres, np. <code>https://twoja-nazwa.onrender.com</code>.
            Adres jest trzymany tylko w pamięci tej karty przeglądarki.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (backendUrlInput.trim()) setBackendUrl(backendUrlInput.trim());
            }}
          >
            <input
              type="text"
              ...
              placeholder="https://twoja-nazwa.onrender.com"
              autoFocus
              style={{
                width: "100%",
                padding: "10px 12px",
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 4,
                fontWeight: 700,
                cursor: backendUrlInput.trim() ? "pointer" : "default",
                fontSize: 14,
                opacity: backendUrlInput.trim() ? 1 : 0.5,
              }}
            >
              Zapisz i kontynuuj
            </button>
